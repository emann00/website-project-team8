...
def is_safe_stock_url(url):
    try:
        ...
        if netloc not in ALLOWED_STOCK_HOSTS:
            return False
...
def check_stock(product_id):
    try:
        api_url = request.form.get("stock_api", "")
        if not is_safe_stock_url(api_url):
            raise ValueError("This stock API URL is not allowed")
...
def image():
    ...
    return send_file(ROOT / "images" / filename)
